...
function validSignature(raw, request) {
  const signature = request.headers?.get ? request.headers.get("x-signature") : request.headers?.["x-signature"];
  const timestamp = request.headers?.get ? request.headers.get("x-timestamp") : request.headers?.["x-timestamp"];
...
export default async function handler(request, response) {
  ...
  if (!validSignature(raw, request)) return json({ ok: false, error: "Invalid signature" }, 401, response);
  try {
    const payload = JSON.parse(raw);
    const eventName = payload?.name || payload?.type || "";
    if (!eventName.includes("payment_link.paid") && !eventName.includes("payment_intent.succeeded")) return json({ received: true }, 200, response);
    const object = payload?.data?.object || payload?.data || {};
    const metadata = object.metadata || {};
    if (metadata.product_key !== "malaysia_manual") return json({ received: true, ignored: "Not malaysia_manual" }, 200, response);
    const orderNo = metadata.order_no || object.reference || object.merchant_order_id;
    if (!orderNo) return json({ received: true, ignored: "Missing order number" }, 200, response);
    ...
    return json({ received: true, manual_fulfillment: true }, 200, response);
  } catch (error) {
    console.error("JP Airwallex webhook failed", error);
    return json({ ok: false, error: "Webhook processing failed" }, 500, response);
  }
}
